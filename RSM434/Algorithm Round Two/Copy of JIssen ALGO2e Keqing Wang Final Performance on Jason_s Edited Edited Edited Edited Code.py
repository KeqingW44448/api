...
s = requests.Session()
s.headers.update({'X-API-key': 'ZVD55BNF'}) # API Key from YOUR RIT Client 66666
    ...
    if threshold > 0.50: #If the total position reaches 95% of the limit
        ...
        for item in securities:
            ...
            if position > 0.0: #Dump long position if the underlaying ticker has positive position
                resp = s.post(u_orders, params = {'ticker': item['ticker'], 'type': 'MARKET', 'quantity': abs(position), 'action': 'SELL'})
                ...
                resp = s.post(u_orders, params = {'ticker': item['ticker'], 'type': 'MARKET', 'quantity': abs(position), 'action': 'BUY'})
...
def main():
    ...
    while status == 'ACTIVE':
        for i in range(3):
            ...
            #     resp = s.post(u_orders, params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT * 1.5, 'price': best_bid_price - 0.60, 'action': 'BUY'})
            #     print('A SUPER BUYERS MARKET FOR ' + str (ticker_symbol) + '! Longing ' + str(ORDER_LIMIT * 1.3) + ' at ' + str(best_bid_price))
            # elif is_super_seller(bid_vol, ask_vol):
            #     #Take advantage when it is a seller's market
            #     resp = s.post(u_orders, params = {'ticker': ticker_symbol, 'type': 'LIMIT', 'quantity': ORDER_LIMIT * 1.5, 'price': best_ask_price + 0.60, 'action': 'SELL'})
            #     print('A SUPER BUYERS MARKET FOR ' + str (ticker_symbol) + '! Shorting ' + str(ORDER_LIMIT * 1.3) + ' at ' + str(best_ask_price))
            if is_competitive(best_bid_price, best_ask_price):
                #To fix the problem of 0 trade in RY when spread is extremely narrow
                resp = s.post(u_orders, params = {'ticker': ticker_list[i], 'type': 'LIMIT', 'quantity': ORDER_LIMIT * 0.9, 'price': best_bid_price - 0.01, 'action': 'BUY'})
                resp = s.post(u_orders, params = {'ticker': ticker_list[i], 'type': 'LIMIT', 'quantity': ORDER_LIMIT * 0.9, 'price': best_ask_price + 0.01, 'action': 'SELL'})
                print('Low spread for ' + str(ticker_list[i]) + ' long & shorting at a reduced price')
            else:
                resp = s.post(u_orders, params = {'ticker': ticker_list[i], 'type': 'LIMIT', 'quantity': ORDER_LIMIT, 'price': best_bid_price - 0.02, 'action': 'BUY'})
                resp = s.post(u_orders, params = {'ticker': ticker_list[i], 'type': 'LIMIT', 'quantity': ORDER_LIMIT, 'price': best_ask_price + 0.02, 'action': 'SELL'})
