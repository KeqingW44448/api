...
def main():
    ...
    while status == "ACTIVE":
        ...
        for i in range(3):
            ...
            if bid > valuation[i]:
                resp = s.post(u_orders, params={'ticker': ticker_list[i], 'type': 'MARKET', 'quantity': 500, 'action': 'SELL'})
            elif ask < valuation[i]:
                resp = s.post(u_orders, params={'ticker': ticker_list[i], 'type': 'MARKET', 'quantity': 500, 'action': 'BUY'})
